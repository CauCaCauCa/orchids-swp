...
async function GetAccountsStats() {
    ...
    const latestAccountCreation = await collection
        .find(
            {},
            { projection: { _id: 0, username: 1, email: 1, created_at: 1 } }
        )
        .sort({ created_at: -1 })
        .limit(1)
        .toArray();
    const accountsCreatedInLast7Days = await collection.countDocuments({
        created_at: { $gte: Math.floor(Date.now() / 1000) - 7 * 24 * 60 * 60 }
    });
    const accountWithMostPosts = await collection
        .find({})
        .sort({ numberPost: -1 })
        .limit(-1)
        .toArray();
    const accountWithMostQuestions = await collection
        .find({})
        .sort({ numberQuestion: -1 })
        .limit(-1)
        .toArray();
    const mostFollowedAccount = await collection
        .aggregate([
            {
                $project: {
                    _id: 0,
    ...
    return {
        ...
        accountWithMostPosts: !accountWithMostPosts
            ? null
            : {
        ...
        accountWithMostQuestions: !accountWithMostQuestions
            ? null
            : {
...
async function ToggleDeactivateAccount(accountEmail) {
    const { collection, close } = await getAccountsCollection();
    const account =
        (await collection.findOne({ email: accountEmail })) ||
        (() => {
...
async function GetPostStats() {
    ...
    const latestPost = await collection
        ...
        .limit(1)
        .toArray();
    const postCreatedInLast7Days = await collection.countDocuments({
        date: { $gte: Math.floor(Date.now() / 1000) - 7 * 24 * 60 * 60 }
    });
    const mostLikedPost = await collection
        .aggregate([
            {
                $project: {
                    _id: 1,
        ...
        .toArray();
    const mostCommentedPost = await collection
        .aggregate([
            {
                $project: {
                    _id: 1,
...
async function GetQuestionsStats() {
    ...
    const totalAnswers = await collection
        .aggregate([
            {
                $project: {
        ...
        .toArray();
    const questionsCreatedInLast7Days = await collection.countDocuments({
        createDate: { $gte: Math.floor(Date.now() / 1000) - 7 * 24 * 60 * 60 }
    });
    const latestQuestion = await collection
        .find({})
        .sort({ createDate: -1 })
        .limit(1)
        .toArray();
