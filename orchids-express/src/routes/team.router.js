 ...
 */
router.get(
    '/fetch/teams-by-account',
    sessionKeyAuth.CheckTimeoutToken,
    async (req, res) => {
...
router.get(
    ...
    async (req, res) => {
        ...
        try {
            ...
            const result = {
                role: await teamService.getAccountRole(email, teamEmail)
            };
...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            const result = await teamService.getMemberSpecific(
                teamEmail,
                memberEmail
            );
 ...
 */
router.post(
    '/:teamEmail/create/post',
    sessionKeyAuth.CheckTimeoutToken,
    ownership.isMemberOfTeam,
    async (req, res) => {
        Logger.info('POST /team/create/:teamEmail/post');
        try {
            const teamEmail = req.params.teamEmail;
            const { title, content, bground } = req.body;
            const result = await teamService.createTeamPost(
                title,
                content,
                bground,
                teamEmail
...
router.get('/:teamEmail/fetch/post', async (req, res) => {
    ...
    try {
        ...
        const result = await teamService.getTeamPostsByTimestamp(
            teamEmail,
            timestamp
        );
...
router.post('/get-list-info-team-by-emails', async (req, res) => {
    try {
        const result = await teamService.getListInfoTeamByEmails(
            req.body.listEmailTeam
        );
        res.status(200);
        res.send(result);
    } catch (error) {
...
router.delete(
    ...
    async (req, res) => {
        ...
        try {
            ...
            const callerEmail = sessionKeyAuth.getEmailFromToken(req, res);
            const result = await teamService.deleteTeamPost(
                postId,
                teamEmail,
                callerEmail
            );
...
router.get('/top/:count', async (req, res) => {
    Logger.log('GET TOP TEAMS');
    try {
        const count = req.params.count || 3;

        const response = await teamService.getTopTeams(count);
