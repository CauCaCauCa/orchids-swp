...
import { DeleteCommentPost, GetPostInfo, LikeCommentPost, UnlikeCommentPost } from '../../api/postAPI'
...
export default function PostPage({ PostData }) {
  ...
  useEffect(() => {
    ...
    if (!PostData) {
      ...
      GetPostInfo(id).then(res => {
        ...
        if (res.ListEmailLiked.includes(localStorage.getItem('email'))) {
          setIsLiked(true);
        }
        setPost(res);
      ...
      // check if user liked this post
      if (PostData.ListEmailLiked.includes(localStorage.getItem('email'))) {
        setIsLiked(true);
      }
      // get list username by emails
  ...
  function likeComment(postId, dateComment, emailCommentor) {
    LikeCommentPost(postId, dateComment, emailCommentor).then(res => {
      if (res.acknowledged == true) {
        post.ListComment.find(comment => comment.date == dateComment && comment.email == emailCommentor)
        ...
        ? (
          <div id="post-page">
            ...
            <div id="head">
              ...
              <div id="author">
                <Avatar id='avatar' src={post.avatar} alt='#'
                  onClick={() => {
                    if(post.isTeam){
                      navigate(`/teams/${post.emailCreator}`)
                    } else {
                      navigate(`/view/user?username=${post.username}`)
                    }
                  }}
                />
                <span style={{ marginRight: '.5rem' }}>@{post.username}</span>  {FormatDate(post.date)}
                {post.emailCreator == localStorage.getItem('email') &&
                  <button style={{ marginLeft: '2rem' }}
                    onClick={() => {
                      if (window.confirm('Bạn muốn chỉnh sửa bài viết này?')) {
                ...
                <button style={{ position: 'absolute', right: '13rem' }}
                  onClick={like}
                >
                  {isLiked ?
                    <i className="fa-solid fa-thumbs-up">{" " + post.ListEmailLiked.length}</i>
                    :
                    <i className="fa-regular fa-thumbs-up">{" " + post.ListEmailLiked.length}</i>
                  }
                </button>
                <button id='share' style={{ position: 'absolute', right: '7rem' }}
                  onClick={() => copyToClipboard(`http://localhost:3000/post-page?id=${post._id}`)}
                >
                  <i className="fa-regular fa-share">{" " + "Share"}</i>
            ...
            <div id="comment-list">
              {usernameList != null ?
                post.ListComment.map((comment, index) => {
                  var user = usernameList.find(username => username.email === comment.email);
                  if (user) {
                    comment.username = user.username;
                    comment.avatar = user.avatar;
                  } else {
                    comment.username = localStorage.getItem('username');
                    comment.avatar = localStorage.getItem('avatar');
                  }
                  return (
                    <>
                      <div className="comment">
                        <div id="author">
                          <Avatar id='avatar' src={comment.avatar} alt='#' />
                          <span style={{ marginRight: '.5rem' }}>@{comment.username}</span>  {FormatDate(comment.date)}
                          {
                            !comment.ListEmailLiked.includes(localStorage.getItem('email')) ?
                              <i className="fa-regular fa-heart fa-lg"
                                onClick={() => likeComment(post._id, comment.date, comment.email)}
                                style={{
                                  color: 'black',
                                  marginLeft: '1rem',
                                  cursor: 'pointer',
                              ...
                              <i className="fa-solid fa-heart fa-lg"
                                onClick={() => unlikeComment(post._id, comment.date, comment.email)}
                                style={{
                                  color: 'red',
                                  marginLeft: '1rem',
                                  cursor: 'pointer',
                          ...
                          {" " + comment.ListEmailLiked.length}
                          {
                            ...
                            <i className="fa-solid fa-trash"
                              style={{ color: "#ff0000", position: 'absolute', right: '10rem', cursor: 'pointer' }}
                              onClick={() => {
                                if (window.confirm('Are you sure you wish to delete this item?')) {
                                  var date = comment.date;
                                  var postId = post._id;
                                  var email = comment.email;
                                  DeleteCommentPost(postId, date).then(res => {
                                    if (res.acknowledged == true) {
                                      showSuccess("Comment deleted successfully.")
                                      var newCommentList = post.ListComment.filter(commentfilter => commentfilter.date != comment.date || commentfilter.email != comment.email);
                                      post.ListComment = newCommentList;
                                      setPost({ ...post });
                                    } else {
                        ...
                        <div id="content" dangerouslySetInnerHTML={{ __html: comment.content }}>
                        </div>
                      </div>
                      <hr style={{ width: '80%' }} />
                    </>
